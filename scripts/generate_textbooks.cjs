...
const publicUnitsDir = path.join(ROOT_DIR, 'public', 'units');
const dataParserSrc = fs.readFileSync(path.join(ROOT_DIR, 'src', 'data_parser.js'), 'utf8');
const dataParserCode = dataParserSrc.replace(/export /g, '');
eval(dataParserCode);

let examGuideSrc = '';
if (fs.existsSync(path.join(ROOT_DIR, 'src', 'exam_guide_content.js'))) {
  examGuideSrc = fs.readFileSync(path.join(ROOT_DIR, 'src', 'exam_guide_content.js'), 'utf8');
...
`;
  ...
  workbooksToGenerate.forEach((period) => {
    ...
    let bannerImageSrc =
      ...
      unitData.banner ||
      '/assets/water_and_sanitation_was_roman_bathhouse.png';
    if (bannerImageSrc) {
      bannerImageSrc =
        typeof resolveAssetPath === 'function'
          ? resolveAssetPath(bannerImageSrc, 2)
    ...
    periodLessons.forEach((lesson, lessonIndex) => {
          ...
          if (block.tasks) {
            block.tasks.forEach((task) => {
              ...
              if (currentUnitId === 'early_modern_world') {
                if (typeof task.text === 'string')
                  task.text = task.text.replace(/^Q\d+[\.:\s]*/i, '');
                if (typeof task.question === 'string')
                  task.question = task.question.replace(/^Q\d+[\.:\s]*/i, '');
                if (typeof task.instruction === 'string')
                  task.instruction = task.instruction.replace(/^Q\d+[\.:\s]*/i, '');
                if (typeof task.instructions === 'string')
                  task.instructions = task.instructions.replace(/^Q\d+[\.:\s]*/i, '');
                if (typeof task.title === 'string')
                  task.title = task.title.replace(/^Q\d+[\.:\s]*/i, '');
              }
            });
          }
        });
      ...
      if (currentUnitId === 'early_modern_world') {
        ...
        if (lesson.primary_source && typeof lesson.primary_source.question === 'string') {
          lesson.primary_source.question = lesson.primary_source.question.replace(
            /^Q\d+[\.:\s]*/i,
            '',
          );
        }
          ...
          imgTags = srcs
            .map((src) => {
              ...
              const style =
                ...
                (srcs.length > 1
                  ? 'max-width: 48%; max-height: 250px; object-fit: contain;  border-radius: 4px; box-shadow: 2px 2px 5px rgba(0,0,0,0.1);'
                  : 'max-width: 100%; max-height: 250px; object-fit: contain;  border-radius: 4px; box-shadow: 2px 2px 5px rgba(0,0,0,0.1);');
        ...
        lesson.narrative_blocks.forEach((block, bIdx) => {
          ...
          if (block.images && Array.isArray(block.images)) {
            ...
            block.images.forEach((imgObj) => {
              ...
              if (rawSrc) {
                let src =
                  typeof resolveAssetPath === 'function' ? resolveAssetPath(rawSrc, 2) : rawSrc;

                let caption = imgObj.image_caption || imgObj.image_alt || '';

                let badgeHtml = '';
                let inlineSourceHtml = imgObj.source_letter
                  ? `<strong>Source ${imgObj.source_letter}:</strong> `
                  : '';
          ...
          if (block.image && (!block.images || block.images.length === 0)) {
            let src =
              typeof resolveAssetPath === 'function'
                ? resolveAssetPath(block.image, 2)
                : block.image;

            let caption = block.image_caption || block.image_alt || '';

            let badgeHtml = '';
            let inlineSourceHtml = block.source_letter
              ? `<strong>Source ${block.source_letter}:</strong> `
              : '';
        ...
        if (lesson.sources && lesson.sources.length > 0 && isGCSE) {
          ...
          lesson.sources.forEach((source, sIdx) => {
            ...
            if (source.src || source.source || source.caption || sourceContent) {
              let containerStyle =
                unitId === 'cme_new'
                  ? `style="flex: 1 1 45%; max-width: 48%; box-sizing: border-box; page-break-inside: avoid; border: 1px solid #ccc; padding: 10px; border-radius: 6px; box-shadow: 1px 1px 4px rgba(0,0,0,0.05); margin-bottom: 0;"`
                  : `style=""`;
              html += `
        ...
        if (lesson.gcse_task) {
          ...
          if (lesson.gcse_task.tasks) {
            lesson.gcse_task.tasks.forEach((task) => {
              ...
              if (task.marks && !_tText.toLowerCase().includes('marks)'))
                _tText += ` (${task.marks} marks)`;
            ...
            lesson.gcse_task.sources.forEach((srcObj) => {
              sourceHTML +=
                '<div style="flex: 1; border: 1px solid #ccc; padding-top: 10px; padding-bottom: 10px; text-align: center; page-break-inside: avoid; break-inside: avoid;">';
              if (srcObj.type === 'visual') {
                let imgSrc =
                  typeof resolveAssetPath === 'function'
                    ? resolveAssetPath(srcObj.src, 2)
                    : srcObj.src;
