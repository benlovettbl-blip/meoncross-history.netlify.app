...
(async () => {
  ...
  const units =
    unitArg === 'all'
      ? fs
          .readdirSync(path.join(publicDir, 'units'))
          .filter((f) => fs.statSync(path.join(publicDir, 'units', f)).isDirectory())
      : [unitArg];
    ...
    if (fs.existsSync(unitDir)) {
      ...
      for (const file of files) {
        ...
        for (const src of imgSrcs) {
          ...
          if (!fs.existsSync(absPath) && unit === 'cme_new') {
            const cleanSrc = src.replace(/^\/+/, '');
            const candidates = [
              path.join(publicDir, cleanSrc),
              path.join(publicDir, 'units', 'cme_new', cleanSrc),
              path.join(publicDir, 'units', 'cme_new', 'assets', path.basename(src)),
              path.join(rootDir, 'units', 'cme_new', 'assets', path.basename(src)),
              path.join(publicDir, 'images', path.basename(src)),
              path.join(publicDir, 'assets', path.basename(src)),
            ];
          ...
          await page.evaluate(
            (oldSrc, newSrc) => {
              document
                .querySelectorAll(`img[src="${oldSrc}"]`)
        ...
        while (!success && retries > 0) {
          try {
            ...
            if (err.code === 'EBUSY' || err.code === 'EPERM') {
              console.warn(
                `File locked (${pdfFileName}). Retrying in 2 seconds... (${retries - 1} retries left)`,
              );
        ...
        if (!success) {
          console.error(
            `Failed to export ${pdfFileName} after multiple retries due to file locking.`,
          );
        }
      }
    }
