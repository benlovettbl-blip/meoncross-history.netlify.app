...
(async () => {
  ...
  await page.goto('file://' + path.resolve('public/units/cme_new/textbook_KT3_PUBLISHER.html'), {
    waitUntil: 'networkidle0',
  });
  const els = await page.$$('.textbook-page');
  console.log(`Found ${els.length} pages in KT3`);

  for (let i = 0; i < els.length; i++) {
    const out = path.join(artDir, `kt3_p${i + 1}.png`);
    await els[i].screenshot({ path: out });
    console.log(`Saved page ${i + 1} to ${out}`);
  }

  await browser.close();
  console.log('All KT3 pages saved.');
})();
