...
export async function auditUnit(unitId) {
  ...
  const unitPath = path.join(process.cwd(), 'units', unitId, 'data.js');
  ...
  if (!unitData || !unitData.lessons) {
    console.warn(`⚠️  Skipping ${unitId}: no lessons array found in data.js`);
    return true; // Not a failure, just an empty/stub unit
  }
  ...
  unitData.lessons.forEach((lesson, lIdx) => {
    ...
    lesson.narrative_blocks.forEach((block, bIdx) => {
      ...
      if (block.tasks && Array.isArray(block.tasks) && block.tasks.length === 0) {
        warnings.push(
          `[EMPTY TASKS] ${loc}: Contains an empty 'tasks: []' array. This will render as an empty task box.`,
        );
      }
      ...
      if (block.tasks && Array.isArray(block.tasks)) {
        block.tasks.forEach((t, tIdx) => {
          ...
          if (!text || text.trim() === '') {
            warnings.push(
              `[MISSING TASK TEXT] ${loc} -> Task ${tIdx}: Task is missing 'question', 'q', or 'text' property.`,
            );
          }
        });
      }
      ...
      if (block.title) {
        ...
        if (
          (titleLower.includes('vocabulary') ||
            titleLower.includes('do now') ||
            titleLower.includes('task')) &&
          (!block.tasks || block.tasks.length === 0)
        ) {
          warnings.push(
            `[ORPHANED TITLE] ${loc}: Title '${block.title}' suggests a task/activity, but no tasks are attached.`,
          );
        }
      }
      ...
      if (
        ...
        if (!textContent.includes('<h') && !textContent.includes('<strong>')) {
          warnings.push(
            `[SUSPICIOUS FRAGMENT] ${loc}: Very short text block with no media or tasks. Is this an orphaned fragment? Text: "${textContent.substring(0, 30)}..."`,
          );
        }
      }
      ...
      textFields.forEach((field) => {
        if (
          field &&
          (field.includes('<strong></strong>') ||
            field.includes('<em></em>') ||
            field.includes('<b></b>'))
        ) {
          warnings.push(
            `[EMPTY TAG] ${loc}: Contains an empty HTML tag (e.g. <strong></strong>), indicating missing data.`,
          );
        }
      });
          ...
          if (sourceMatch) {
            sourceMatch.forEach((src) => {
              ...
              if (!sourceFoundInLesson) {
                warnings.push(
                  `[SOURCE MISMATCH] ${loc} -> Task ${tIdx}: Task references '${src}', but this source label is missing from the entire lesson.`,
                );
              }
            });
          }
      ...
      if (b.images && Array.isArray(b.images)) {
        b.images.forEach((img) => {
          if (img.source_letter) {
            definedSources.push({
              letter: img.source_letter,
              blockIdx: bIdx,
              hasOwnTasks,
              isImagesArray: true,
              arrayLength: b.images.length,
            });
          }
        });
      }
    ...
    definedSources.forEach((src) => {
      ...
      if (!isExplicitlyMentioned) {
        if (src.isImagesArray && src.arrayLength > 1) {
          warnings.push(
            `[ORPHANED SOURCE] Lesson ${lIdx + 1} -> Block ${src.blockIdx}: Source ${src.letter} is in an image array but never explicitly asked about in any task.`,
          );
        } else if (!src.hasOwnTasks) {
          warnings.push(
            `[ORPHANED SOURCE] Lesson ${lIdx + 1} -> Block ${src.blockIdx}: Source ${src.letter} is defined, but has no tasks attached to it and is never explicitly asked about.`,
          );
        }
      }
    });
