...
import { generateWorkbookHtml, generateBulkWorkbookHtml, TIMELINE_WORKSHEET_PROMPTS, TIMELINE_KEYWORDS, getBlankedAndAnswers } from './workbook_html_generator.js';
...
export function renderMasteryView(subtopicId) {
  ...
  if (isCoreMode && data.scaffoldedPractice) {
    ...
    if (sp.questionType === 'consequence') {
      ...
      formulaSteps = [
        `<strong>Point:</strong> Clearly state the consequence. (e.g., <em>"One consequence was..."</em>)`,
        `<strong>Evidence:</strong> Add specific facts, dates, or statistics.`,
        `<strong>Explain:</strong> Explain what this led to or meant.`
      ];
      ...
      formulaSteps = [
        `<strong>Point:</strong> State one reason why the factor/event was important.`,
        `<strong>Evidence:</strong> Support this with specific historical facts and details.`,
        `<strong>Explain:</strong> Explain how this impacted the wider outcome or course of events.`
      ];
    }
    ...
    const stepTitles = {
      point: sp.questionType === 'narrative' ? 'Select the Beginning (Part 1)' : 'Select the Point',
      evidence: sp.questionType === 'narrative' ? 'Select the Middle / Link (Part 2)' : 'Select the Evidence',
      explain: sp.questionType === 'narrative' ? 'Select the End / Result (Part 3)' : 'Select the Explanation'
    };
...
<head>
  ...
  <style>
    ...
    body { font-family: 'Arial', sans-serif; font-size: 12pt; color: #000000; line-height: 1.4; background: #ffffff; margin: 0; padding: 0; }
