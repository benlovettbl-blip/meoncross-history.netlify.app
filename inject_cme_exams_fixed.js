...
const blocks = data.exam_blocks;
const findByTitle = (str) => blocks.find(b => b.title.includes(str));

const block1 = findByTitle('1.2 & 1.3'); // KT1.2 & KT1.3
if (block1) {
  if (!block1.questions) block1.questions = [];
  block1.questions.push({
    type: 'consequence_4',
    marks: 4,
...
const block2 = findByTitle('2.1 & 2.2'); // KT2.2
if (block2) {
  if (!block2.questions) block2.questions = [];
  block2.questions.push({
    type: 'importance_8',
    marks: 8,
  ...
  block2.questions.push({
    ...
    structure_strip: { starters: ['In the aftermath of the devastating Six Day War in 1967...', 'As a direct result of this diplomatic failure and their displacement...', 'Consequently, this horrifying spectacle on the global stage...'], fact_bank: ['Resolution 242', 'Khartoum', 'Three No’s', 'Black September', 'Munich', 'Wrath of God'] }
  });
}

const block3 = findByTitle('3.1 & 3.2'); // KT3.1 & KT3.2
if (block3) {
  if (!block3.questions) block3.questions = [];
  block3.questions.push({
    type: 'importance_8',
    marks: 8,
...
const block4 = findByTitle('3.2 & 3.3'); // KT3.3
if (block4) {
  if (!block4.questions) block4.questions = [];
  block4.questions.push({
    type: 'importance_8',
    marks: 8,
...
console.log('Successfully injected real questions into correct exam blocks');
