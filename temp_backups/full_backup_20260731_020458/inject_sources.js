...
const sources = [
    {
        ...
        teacher_notes_primer: "A pivotal photograph showing Ben-Gurion reading the declaration under the portrait of Theodor Herzl.",
        teacher_notes_question: "Why might the presence of Theodor Herzl's portrait in the background be significant to the audience of this declaration?"
    },
    {
        lessonIdx: 1,
        ...
        teacher_notes_primer: "This image highlights the mass displacement of Palestinians, which became the central issue of the Nakba.",
        teacher_notes_question: "How does this photograph challenge or support the Israeli narrative of the 1948 war?"
    },
    {
        lessonIdx: 2,
        ...
        teacher_notes_primer: "Nasser's charismatic leadership united the Arab world but deeply alarmed Britain, France, and Israel.",
        teacher_notes_question: "How does Nasser's confident posture reflect his political ambitions in the Middle East during this period?"
    },
    {
        lessonIdx: 3,
        ...
        teacher_notes_primer: "A deeply iconic image in Israeli history, symbolizing the reunification of Jerusalem and Jewish access to their holiest site.",
        teacher_notes_question: "What emotions are conveyed by the soldiers, and why was this specific location so strategically and culturally vital?"
    },
    {
        lessonIdx: 4,
        ...
        teacher_notes_primer: "This map visually demonstrates the massive expansion of Israeli-controlled territory, which created the geopolitical reality still contested today.",
        teacher_notes_question: "Based on the map, which Arab nation suffered the greatest territorial loss, and how might that affect future relations?"
    },
    {
        lessonIdx: 5,
        ...
        teacher_notes_primer: "The image shows the heavy mechanized warfare that characterized the 1973 conflict.",
        teacher_notes_question: "How does this photograph reflect the wider Cold War context of the Arab-Israeli conflict?"
    },
    {
        lessonIdx: 6,
        query: "Camp David Accords Begin Sadat",
        ...
        teacher_notes_primer: "This historic handshake ended decades of war between Egypt and Israel but alienated Egypt from the rest of the Arab world.",
        teacher_notes_question: "Why was the role of the US President (Jimmy Carter) so critical in achieving this handshake?"
    },
    {
        lessonIdx: 7,
        query: "Yasser Arafat PLO UN",
        ...
        teacher_notes_primer: "Arafat became the face of the Palestinian struggle, utilizing both diplomacy and armed resistance.",
        teacher_notes_question: "How did Arafat's leadership style shift between the 1970s and the 1990s?"
    },
    {
        lessonIdx: 8,
        query: "Oslo Accords 1993 handshake",
        ...
        teacher_notes_primer: "The handshake symbolized a monumental shift toward a two-state solution, though the peace process ultimately stalled.",
        teacher_notes_question: "Does this photograph suggest that peace was inevitable, or does the body language hint at underlying hesitations?"
    }
];
...
async function run() {
    ...
    // Inject into data.js
    // I will regex replace `title: "KT1.1: ` or similar to inject the primary_source and teacher_notes.
    const { unitData } = require('./public/units/cme_new/data.js');
    
    for (const source of sources) {
        const lessonObj = unitData.lessons[source.lessonIdx];
        if (!lessonObj) continue;
        const lessonTitleEscaped = lessonObj.title.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        
        let injectStr = `
      primary_source: {
        title: "${source.title}",
        src: "/units/cme_new/assets/${source.filename}",
        caption: "${source.caption}",
        question: "${source.question}"
      },`;
        
        // Ensure teacher_notes is modified per the rules
        // The rule says: "Whenever generating or modifying source_context in teacher_notes for a visual source, you MUST always finish the context blurb with a targeted 'Hinge Question' to spark class discussion about the source."
        
        // Find if teacher_notes exists, if not, create it. 
        // In the text, let's just do a regex replace to add primary_source.
        // And we can also inject teacher_notes.
        
        let regex = new RegExp(`(title:\\s*"${lessonTitleEscaped}"\\s*,)`);
        
        // Wait, the data.js might already have teacher_notes.
        // It's safer to just modify the object and write back via stringify?
        // NO! ES modules can't be easily JSON.stringified if they have functions.
        // Does cme_new/data.js have functions? Let's check!
    }
    
    console.log("Images fetched. Ready for data.js injection.");
}
