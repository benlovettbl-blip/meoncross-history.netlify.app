    ...
    if (lesson.do_now) {
      if (lesson.do_now.type === "timeline") {
        html += `<div class="tasks-section do-now-section">
          <h3><i class="fa-solid fa-clock-rotate-left"></i> Timeline Recall</h3>
      ...
      lesson.tasks.forEach((task, tIdx) => {
        html += `<div class="task-item">
          ...
          <div class="task-actions" style="margin-top: 10px;">
            ${task.starter ? `<button class="btn btn-scaffold" onclick="document.getElementById('scaffold-starter-${tIdx}').style.display='block'"><i class="fa-solid fa-play"></i> Starter</button>` : ''}
            ${task.clue ? `<button class="btn btn-scaffold" onclick="document.getElementById('scaffold-clue-${tIdx}').style.display='block'"><i class="fa-solid fa-question"></i> Clue</button>` : ''}
            ${task.model ? `<button class="btn btn-scaffold" onclick="document.getElementById('scaffold-model-${tIdx}').style.display='block'"><i class="fa-solid fa-check-double"></i> Model Answer</button>` : ''}
          </div>
          <div id="scaffold-starter-${tIdx}" class="scaffold-box starter" style="display:none;"><i class="fa-solid fa-play"></i> <strong>Start your sentence with:</strong> ${task.starter}</div>
          <div id="scaffold-clue-${tIdx}" class="scaffold-box clue" style="display:none;"><i class="fa-solid fa-lightbulb"></i> <strong>Hint:</strong> ${task.clue}</div>
          <div id="scaffold-model-${tIdx}" class="scaffold-box model" style="display:none;"><i class="fa-solid fa-star"></i> <strong>WAGOLL:</strong> ${task.model}</div>
        </div>`;
      });
      html += `</div>`;
    }
