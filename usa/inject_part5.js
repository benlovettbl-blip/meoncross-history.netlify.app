...
let originalCode = fs.readFileSync('generate_foundation_pack.js', 'utf8');

const topicsRegex = /const topics = \[[\s\S]*?\];\n\nfunction/;
originalCode = originalCode.replace(topicsRegex, "const topics = " + JSON.stringify(topics, null, 2) + ";\n\nfunction");

const newGeneratePages = `function generatePages() {
...
const generatePagesRegex = /function generatePages\(\) \{[\s\S]*?\}\n/g;
originalCode = originalCode.replace(generatePagesRegex, newGeneratePages);

fs.writeFileSync('generate_foundation_pack.js', originalCode);
console.log("Successfully updated generate_foundation_pack.js");
