...
// 2. Nomenclature
code = code.replace(/GCSE History Hub/g, 'The History Department');

// 3. Year
code = code.replace(
  /Points for Improvement for 2026\/2027/g,
  'Points for Improvement for 2026/2027',
); // unchanged, but let's update subtitle
code = code.replace(
...
code = code.replace(/<td>\s*<strong>\$\{discFocus\}<\/strong>\s*<\/td>/, '');
