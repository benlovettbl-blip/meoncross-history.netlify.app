...
function applyFinalPatches() {
    ...
    // We can use a regex to replace this specific block.
    // In generate_pupil_workbooks.js, it looks like:
    ...
    // }
    gpw = gpw.replace(
        /if\s*\(\s*lesson\.extended\s*&&\s*lesson\.extended\.question\s*\)\s*\{\s*allExamTasksHtml\s*\+=\s*`<div style="margin-bottom: 10px;"><strong>Q\.\s*\$\{formatText\(lesson\.extended\.question\)\}<\/strong><\/div>`;\s*\}/g,
        `if (lesson.extended && lesson.extended.question) {
    ...
    // 6. Filter out floating text (content leak)
