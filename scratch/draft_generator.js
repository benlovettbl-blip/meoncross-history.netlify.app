...
const periods = [
  { name: 'medieval', title: 'Medieval (c1250-c1500)', filter: l => l.id.startsWith('lesson_1_'), image: 'medieval_pano_1784551792993.png' },
  { name: 'renaissance', title: 'Renaissance (c1500-c1700)', filter: l => l.id.startsWith('lesson_2_'), image: 'renaissance_pano_1784551804068.png' },
  { name: '18th_19th', title: '18th & 19th Century Medicine', filter: l => l.id.startsWith('lesson_3_'), image: 'industrial_pano_1784551813599.png' },
  { name: 'modern', title: 'Modern (c1900-present)', filter: l => l.id.startsWith('lesson_4_'), image: 'modern_pano_1784551822373.png' },
  { name: 'western_front', title: 'Western Front', filter: l => l.id.startsWith('lesson_5_'), image: 'western_front_pano_1784551831887.png' }
];
...
`;
  ...
  <div style="display: flex; gap: 30px; margin: 25px 5% 0 5%; width: 90%; align-items: stretch;">
    ...
    <div style="flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center;">
      ${Array.isArray(unitData.cover_image) ? 
        `<div style="display: flex; gap: 10px; justify-content: center; align-items: center; height: 230px;">
          ${unitData.cover_image.map(img => `<img src="${img}" style="max-height: 100%; max-width: 32%; object-fit: contain; border: 3px solid #1a237e; border-radius: 4px; box-shadow: 4px 4px 8px rgba(0,0,0,0.2);" alt="${periodTitle}">`).join('')}
        </div>`
        : 
        `<img src="${unitData.cover_image || ''}" style="max-width: 100%; max-height: 230px; border: 3px solid #1a237e; border-radius: 4px; box-shadow: 4px 4px 8px rgba(0,0,0,0.2);" alt="${periodTitle}">`
      }
...
fs.writeFileSync(path.join(__dirname, `draft_workbook_${period.name}.html`), html);
console.log(`Draft generated at scratch/draft_workbook_${period.name}.html`);
});
