...
// Output to a scratch file so it doesn't overwrite the original yet!
content = content.replace(`fs.writeFileSync(path.join(__dirname, \`workbook_\${periodName}.html\`), html);`, `fs.writeFileSync(path.join(__dirname, \`../scratch/draft_workbook_\${periodName}.html\`), html);`);
fs.writeFileSync(path.join(__dirname, 'draft_generate_worksheets.js'), content);
console.log('Created draft script: draft_generate_worksheets.js');
