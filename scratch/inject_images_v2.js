...
const filePath = path.join(rootDir, 'cme_new', 'data.js');
...
function addSource(lessonId, src, title, caption) {
    ...
    if (!lesson.sources.find(s => s.src === src)) {
        ...
        console.log(`${src} already exists in ${lessonId}`);
    }
}
...
// 3. Yom Kippur -> Lesson 7
addSource('lesson_7', 'assets/yom_kippur_crossing.png', 'Reference Image: The Yom Kippur War (1973)', 'Egyptian forces launching Operation Badr, successfully crossing the Suez Canal and breaching the Bar Lev Line on 6 October 1973.');
...
// Wait, looking at the lessons: lesson_8 is "The Palestinian Issue, 1974-1993" so yes, Intifada belongs here.
