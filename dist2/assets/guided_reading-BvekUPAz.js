import{t as e}from"./unit-Duz6193w.js";function t(t,n,r){let i=r?r.currentLessonIndex:0,a=n.find(e=>e.lesson_index===i);if(!a&&n.length>0&&(a=n[0]),!a){t.innerHTML=`<div style="padding: 2rem; text-align: center; color: #64748b;">No guided reading extract available for this lesson.</div>`;return}t.innerHTML=`
    <div class="guided-reading-container" style="background: #fff; padding: 30px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.05); border: 1px solid #e2e8f0; margin-top: 20px;">
      ...
      <div style="display: flex; gap: 30px; align-items: flex-start; margin-bottom: 30px; flex-wrap: wrap;">
        <img src="${e(a.cover_image)}" alt="Book Cover" style="width: 150px; height: auto; border-radius: 6px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
        
        <div style="flex: 1; min-width: 300px;">
          <h2 style="margin: 0 0 10px 0; color: #1e293b; font-size: 2rem; font-family: 'Playfair Display', serif;">${a.book_title}</h2>
          <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;">
            <span style="font-size: 1.2rem; color: #475569;"><i class="fa-solid fa-pen-nib"></i> ${a.author}</span>
            <button class="btn btn-secondary" onclick="document.getElementById('author-context-${a.lesson_index}').classList.toggle('visible')" style="padding: 4px 10px; font-size: 0.85rem; background: #f8fafc; color: #3b82f6; border: 1px solid #bfdbfe;">
              <i class="fa-solid fa-circle-info"></i> Author Context
            </button>
          </div>
          <div style="margin-bottom: 20px;">
            ${a.is_adapted===void 0?``:a.is_adapted?`<div style="display: inline-block; padding: 4px 10px; background: #fffbeb; border: 1px solid #fde68a; border-radius: 4px; font-size: 0.85rem; color: #b45309; font-style: italic;"><i class="fa-solid fa-triangle-exclamation" style="color: #f59e0b;"></i> Historical Adaptation: Rewritten for the classroom based on themes from the original work.</div>`:`<div style="display: inline-block; padding: 4px 10px; background: #ecfdf5; border: 1px solid #a7f3d0; border-radius: 4px; font-size: 0.85rem; color: #047857; font-style: italic;"><i class="fa-solid fa-book-open-reader" style="color: #10b981;"></i> Original Source: An exact, word-for-word extract from the public domain text.</div>`}
          </div>

          <div id="author-context-${a.lesson_index}" style="display: none; margin-bottom: 20px; padding: 15px; background: #eff6ff; border-left: 4px solid #3b82f6; border-radius: 0 6px 6px 0; font-size: 0.95rem; color: #1e3a8a;">
            ${a.author_context}
          </div>

          <div style="margin-bottom: 20px;">
            <button class="btn btn-primary" onclick="const a = document.getElementById('audio-${a.lesson_index}'); if(a.paused){a.play(); this.innerHTML='<i class=\\\'fa-solid fa-pause\\\'></i> Pause Reading';}else{a.pause(); this.innerHTML='<i class=\\\'fa-solid fa-play\\\'></i> Play Audio Reading';}" style="display: inline-flex; align-items: center; gap: 10px; padding: 8px 16px;">
              <i class="fa-solid fa-play"></i> Play Audio Reading
            </button>
            <audio id="audio-${a.lesson_index}" src="${e(a.audio_file)}" onended="this.previousElementSibling.innerHTML='<i class=\\\'fa-solid fa-play\\\'></i> Play Audio Reading';"></audio>
          </div>
          ...
          <div style="background: #fdf2f8; padding: 25px; border-radius: 8px; border: 1px solid #fbcfe8; margin-bottom: 10px;">
            ...
            <p style="color: #831843; font-size: 1.15rem; font-weight: 600; line-height: 1.5; margin: 0;">
              ${a.hinge_question?a.hinge_question:`Discuss the main argument the author makes in this extract. Do you agree?`}
            </p>
      ...
      <div class="reading-extract-container" style="position: relative;">
        <div class="reading-extract" style="font-family: 'Playfair Display', serif; font-size: 1.25rem; line-height: 1.8; color: #1e293b; padding: 30px; background: #fafafa; border-radius: 8px; border: 1px solid #e2e8f0; max-height: 600px; overflow-y: auto;">
          ${a.extract.split(/\n+/).map(e=>`<p style="margin-bottom: 1.5rem;">${e}</p>`).join(``)}
        </div>
  ...
  `;let o=document.createElement(`style`);o.innerHTML=`
    #author-context-${a.lesson_index}.visible {
  ...
  `,t.appendChild(o)}export{t as initGuidedReadingTask};