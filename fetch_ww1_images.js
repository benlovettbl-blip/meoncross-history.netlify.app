...
const searches = [
  ...
  { term: "munitionettes chilwell", filename: "gw_munitionettes.jpg" },
  { term: "Big Four Versailles", filename: "gw_big_three_versailles.jpg" },
  { term: "Clemenceau The Vampire cartoon", filename: "gw_versailles_cartoon.jpg" },
...
async function searchAndDownload(searchObj) {
  return new Promise((resolve, reject) => {
    ...
    https.get(searchUrl, { headers: { 'User-Agent': 'Nodejs/WWIScript (user@example.com)' } }, (res) => {
      let data = '';
      ...
      res.on('end', () => {
        try {
          ...
          const thumbUrl = firstPage.imageinfo[0].thumburl || firstPage.imageinfo[0].url;
...
function downloadImage(url, dest) {
  return new Promise((resolve, reject) => {
    https.get(url, { headers: { 'User-Agent': 'Nodejs/WWIScript (user@example.com)' } }, (res) => {
