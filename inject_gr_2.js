...
// Replace the existing guided_reading property with the updated one
const existingGuidedReadingRegex = /"guided_reading":\s*\[[\s\S]*?\]\n  \},/g;
if (data.match(existingGuidedReadingRegex)) {
    data = data.replace(existingGuidedReadingRegex, '"guided_reading": ' + JSON.stringify(guidedReadingData, null, 2) + '\n  },');
} else {
    // If it doesn't match the exact regex for some reason, we can just replace everything from "guided_reading" to the end of the array.
    const startIdx = data.indexOf('"guided_reading":');
    if (startIdx !== -1) {
        // Find the matching end bracket for guided_reading
        let depth = 0;
        let endIdx = -1;
        for (let i = startIdx + 17; i < data.length; i++) {
            if (data[i] === '[') depth++;
            if (data[i] === ']') {
                depth--;
                if (depth === 0) {
        ...
        if (endIdx !== -1) {
            data = data.substring(0, startIdx) + '"guided_reading": ' + JSON.stringify(guidedReadingData, null, 2) + data.substring(endIdx);
        }
    } else {
...
console.log('Successfully updated guided reading with Lesson titles');
