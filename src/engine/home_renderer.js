...
export function renderHomepage() {
    ...
    if (appStore.state.activeUnitData.type === 'trip') {
      const coverImage = appStore.state.activeUnitData.cover_image ? getAssetUrl(appStore.state.activeUnitData.cover_image) : '';
      ...
      let heroImage = appStore.state.activeUnitData.homepage_background || (typeof appStore.state.activeUnitData.cover_image === 'string' ? appStore.state.activeUnitData.cover_image : null);
      
      if (heroImage && !appStore.state.activeUnitData.cover_sources && !Array.isArray(appStore.state.activeUnitData.cover_image)) {
...
export function renderSidebar() {
    ...
    if (appStore.state.activeUnitData.specification_file) {
      ...
      const specTitle = (appStore.state.activeUnitData.title && appStore.state.activeUnitData.title.includes('KS3')) ? 'Curriculum Overview' : 'Exam Specification';
    ...
    if (appStore.state.activeUnitData.type !== 'trip' && (!appStore.state.activeUnitData.title || !appStore.state.activeUnitData.title.includes('KS3'))) {
    ...
    if (appStore.state.activeUnitData.type !== 'trip' && window.currentUnitId !== 'medieval_england' && window.currentUnitId !== 'early_modern_world' && window.currentUnitId !== 'industrialisation_and_empire' && window.currentUnitId !== 'australia') {
      const examPracticeLink = document.createElement('a');
      examPracticeLink.className = 'lesson-link';
      examPracticeLink.innerHTML = (appStore.state.activeUnitData.title && appStore.state.activeUnitData.title.includes('KS3')) ? '✍️ Assessments' : '✍️ Assessments & Exam Practice';
...
export function renderExamGuide() {
    ...
    let contentHtml = '';
    if (appStore.state.activeUnitData.title && appStore.state.activeUnitData.title.toLowerCase().includes('medicine')) {
      contentHtml = `
      ...
      `;
    } else if (appStore.state.activeUnitData.title && appStore.state.activeUnitData.title.toLowerCase().includes('middle east')) {
      contentHtml = `
      ...
      `;
    } else if (appStore.state.activeUnitData.title && (appStore.state.activeUnitData.title.toLowerCase().includes('weimar') || appStore.state.activeUnitData.title.toLowerCase().includes('germany'))) {
      contentHtml = `
      ...
      `;
    } else if (appStore.state.activeUnitData.title && (appStore.state.activeUnitData.title.toLowerCase().includes('elizabeth') || appStore.state.activeUnitData.title.toLowerCase().includes('armada'))) {
      contentHtml = `
