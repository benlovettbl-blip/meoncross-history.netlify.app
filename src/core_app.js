...
import {
  ...
  generateKeyIndividualCardHTML,
  generateKeyIndividualEmbedHTML,
} from './key_individuals.js';
    ...
    window.addEventListener('renderLessonEvent', (e) => {
      ...
      setTimeout(() => {
        ...
        if (ca) {
          ...
          (document.getElementById('content-area') || window).scrollTo({
            top: 0,
            behavior: 'smooth',
          });
        }
  ...
  `;
    ...
    if (headerActions) {
      ...
      btnTeacher.addEventListener('click', () => {
        ...
        btnTeacher.innerHTML = isActive
          ? '<i class="fa-solid fa-user-tie"></i> Teacher Mode: ON'
          : '<i class="fa-solid fa-user-tie"></i> Teacher Mode';
      ...
      btnCurriculum.innerHTML =
        '<i class="fa-solid fa-clock-rotate-left"></i> Prior Knowledge (Teachers)';
...
window.checkDebate = function (lessonId) {
  ...
  if (!allSorted) {
    ...
    feedback.innerText =
      'Excellent! All evidence sorted correctly. You are ready to write your essay!';
  }
};
...
// --- Historical Figure Popover Logic ---
const KNOWN_HISTORICAL_FIGURES = {
  niccolo_barbaro: {
    name: 'Nicolò Barbaro',
    role: 'Venetian Eye-Witness & Ship Physician',
    lifespan: '1420 – 1494',
    bio: 'A Venetian doctor who was trapped inside Constantinople during the 1453 siege and kept a harrowing day-by-day diary of its fall.',
    image: '/images/individuals/niccol_barbaro.jpg',
  },
  mehmed_ii: {
    name: 'Sultan Mehmed II',
    role: 'Ottoman Sultan ("The Conqueror")',
    lifespan: '1432 – 1481',
...
function initHistoricalFigurePopover() {
  ...
  const showFigurePopover = (e) => {
    ...
    if (!bio || !image) {
      if (personKey && KNOWN_HISTORICAL_FIGURES[personKey]) {
        ...
        if (typeof unitData !== 'undefined' && unitData?.key_individuals) {
          unitIndividuals = unitData.key_individuals;
        } else if (window.unitData?.key_individuals) {
          unitIndividuals = window.unitData.key_individuals;
        } else if (window.appStore?.state?.activeUnitData?.key_individuals) {
        ...
        if (unitIndividuals) {
          const match = unitIndividuals.find(
            (p) =>
              (personKey && (p.id === personKey || p.key === personKey)) ||
...
window.unlockMission = function (btnElement, siteId) {
  ...
  if (
    btnElement &&
    !btnElement.classList.contains('geo-btn-loading') &&
    !btnElement.closest('h4')
  ) {
