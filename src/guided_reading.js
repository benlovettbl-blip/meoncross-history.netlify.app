...
export function initGuidedReadingTask(container, guidedReadingData, globalState) {
  ...
  function renderReadingContent(target, readingData) {
    target.innerHTML = `
      <div class="guided-reading-container" style="background: #fff; padding: 30px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.05); border: 1px solid #e2e8f0;">
    ...
    if (!document.getElementById('guided-reading-style')) {
      ...
      // If the style element already exists, we still need to make sure the toggle rule applies for the new lesson_index.
