...
const regex = /"items": \[\s*"1\. Name two major powers in Europe in 1870\.",\s*"2\. What is an empire\?",\s*"3\. Why is having a strong army important for a country surrounded by others\?",\s*"4\. Define the term 'Balance of Power\.'"\s*\]/m;
...
if (regex.test(content)) {
    content = content.replace(regex, goodDoNow);
    fs.writeFileSync(file, content);
    console.log("Successfully fixed the do_now items in great_war/data.js using Regex");
} else {
    console.log("Could not find the bad do_now array with Regex.");
}
